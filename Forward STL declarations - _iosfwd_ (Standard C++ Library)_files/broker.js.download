if(typeof(COMSCORE)=="undefined"){var COMSCORE={};}if(typeof COMSCORE.SiteRecruit=="undefined"){COMSCORE.SiteRecruit={sv:"scor",testUrl:"broker-test.js",configUrl:"broker-config.js",builderUrl:"builder.js",allowScriptCaching:false,CONSTANTS:{COOKIE_TYPE:{ALREADY_ASKED:1,DD_IN_PROGRESS:2},STATE_NAME:{IDLE:"IDLE",DDINPROGRESS:"DDINPROGRESS"}}};
...
}}return C;},getDevice:function(){var E=new RegExp("iphone|ipad|ipod|android|opera mini|blackberry|windows (phone|ce)|iemobile|htc|nokia|bb10|wpdesktop|lumia|playbook|tablet|mobile","i");
...
var H=L.split(",");var J=C.split(",");var E=G.split(",");var B={};B.sv=E[0].substring(E[0].indexOf(":")+1);
...
}if(!E.cookieoptions){E.cookieoptions=this.getDefaultCookieOptions();}var H={};H.sv="scor";
...
if(C&&C.sv){return C;}else{}}return null;
...
}else{if(M.gracePeriod){H=M.gracePeriod;}else{H=7;}}if(H){H=parseInt(H);var K=2*H*1000;F=(N<K);
...
C.executingPath=A.getExecutingPath("broker.js");if(/cs\_tstMode/i.test(window.location)){
A.loadScript("https://www.microsoft.com/library/svy/"+C.testUrl,true);}if(C.browser.ie||C.browser.mozilla){A.loadScript(C.executingPath+C.configUrl,true);
...
}else{return;}}if(C.sv!==this.config.sv){return;}var G=this.config.testMode;
