...
// App library
var ContentLoader = require('modules/content');
var Editor = require('modules/edit');
var Browser = require('modules/browser');
var Newsletter = require('modules/newsletter');
var TenYears = require('modules/tenyears');
var Svg = require('modules/svg');
...
var Gitguide = function() {
  ...
  var contextPath = "";
  if (('undefined' !== typeof window) && ('undefined' !== typeof window.HTTP_PREFIX)) {
  ...
  // Initialize browser interactions
  var browser = new Browser();
  var newsletter = new Newsletter();
  var tenYears = new TenYears();
  var svg = new Svg();
  var nav = new Navigation(window.location.href);
...
},{"modules/browser":4,"modules/content":5,"modules/edit":6,"modules/navigation":8,"modules/newsletter":9,"modules/svg":11,"modules/tenyears":12}],4:[function(require,module,exports){
var _ = require('lodash');

function Browser() {
  this.HOVER_CLASS = "hoverable";
  this.SCROLL_CLASS = "scrolled";
  this.DESKTOP_MIN = 1025;
...
Browser.prototype.checkMobile = function() {
  return window.innerWidth < this.DESKTOP_MIN;
};

Browser.prototype.isTutorialPage = function() {
  return $("#related-post-sidebar").length;
};

Browser.prototype.isTenYearsPage = function() {
  return $(".ten-years-index").length;
};

Browser.prototype.bindEvents = function() {
  var self = this;

  if (!this.isMobile) {
    $("body").addClass(this.HOVER_CLASS);
  }
  ...
  $(document).on("submit", ".search-page form", function(e) {
    // disable submit on search form, doesn't do anything
    return false;
  });

  self.initRelatedPost();

  var throttleFx = {

     // Throttle adding a hoverable class back to the body
    _hoverThrottle: _.debounce(function() {
      $("body").addClass(self.HOVER_CLASS);
    }, 100),

    // Throttle checking scroll progress
    _scrollThrottle: _.throttle(function() {
      var scrollY = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;
      if (scrollY > window.innerHeight * 0.15) {
        $("body").addClass(self.SCROLL_CLASS);
      } else {
        $("body").removeClass(self.SCROLL_CLASS);
      }
      self.relatedPostScroll();
      self.scrollTenYears();
    }, 100),

    _resizeThrottle: _.throttle(function() {
  ...
  var eventFx = {
    _onScroll: function() {
      $("body").removeClass(self.HOVER_CLASS);

      throttleFx._hoverThrottle();
      throttleFx._scrollThrottle();
    },

    _onResize: function() {
      throttleFx._resizeThrottle();
    }
  };

  window.addEventListener('scroll', eventFx._onScroll);
  window.addEventListener('resize', eventFx._onResize);

};

Browser.prototype.initRelatedPost = function() {

  if (!this.isTutorialPage() || !document.getElementById("related-post-sidebar")) { return; }

  document.getElementById("related-post-sidebar").className = "related-post attached";
  var relatedPostElement = document.getElementById("related-post-sidebar");
  var relatedPostFlyout = relatedPostElement.cloneNode(true);
  relatedPostFlyout.id = "related-post-sidebar-flyout";
  document.getElementById("content").appendChild(relatedPostFlyout);

}

Browser.prototype.relatedPostScroll = function() {
  ...
  var contentHeight = $("#content > .g-row").height();
  var relatedPostHeight = $("#related-post-sidebar").height();
  var maxThreshold = (contentHeight - relatedPostHeight) * .95;
  ...
  if (!document.getElementById("related-post-sidebar-flyout")) {
    this.initRelatedPost();
  }
  document.getElementById("related-post-sidebar-flyout").className = "related-post " + activeClassName;
};

Browser.prototype.scrollTenYears = function() {

  if (!this.isTenYearsPage()) { return; }

  var scrollTop = window.pageYOffset || document.documentElement.scrollTop;
  var viewPortScroll = scrollTop + window.innerHeight;

  $(".timeline .item").each(function() {
    var $item = $(this);
    var itemScrollTop = $item.offset().top;
    if (!$item.hasClass("enter")) {
      if (viewPortScroll >= itemScrollTop) {
        $item.addClass("active enter");
      }
    }
  });
...
var TenYears = require('modules/tenyears');
var Svg = require('modules/svg');
var SearchLoader = require('modules/search');
var _ = require('lodash');
var history_polyfill = require('html5-history-api');

function ContentLoader(contextPath) {
...
ContentLoader.prototype.checkLocalizedDomain = function() {
...
ContentLoader.prototype.initialize = function() {
  var self = this;
...
ContentLoader.prototype.getContent = function() {
  ...
  .done(function(json) {
    if ("undefined" !== typeof json["pages"]) {
      ...
      // Initialize Search
      var sl = new SearchLoader(json.pages);
    }
  });
};

ContentLoader.prototype.bindRoutes = function() {
  var self = this;
  ...
  window.addEventListener('popstate', function(event) {
    ...
    self.parseLink(newPageUrl, true);
  });


};

ContentLoader.prototype.parseLink = function(href, disablePushState) {
  var self = this;
...
ContentLoader.prototype.dispatchPage = function(pageKey, href, disablePushState) {

  pageContent = this.pages[pageKey];
...
ContentLoader.prototype.renderContent = function(domList, href) {
 ...
 */
ContentLoader.prototype.jsonToDom = function(elementDef, parentNode) {
  return this.createNode( elementDef, parentNode, parentNode ? parentNode.ownerDocument : global.document );
};

ContentLoader.prototype.createNode = function( obj, parent, context ) {
...
ContentLoader.prototype.addAttr = function(el, attr, value, context) {
...
ContentLoader.prototype.scrollTo = function($elem, href) {
  if (!$elem.length) {
    return false;
  }
  ...
  window.setTimeout(function() {
    ...
    var pageTitle = $elem.text().substr(0, 25);

  }, 100);
};

ContentLoader.prototype.setUXState = function(flags) {
  var self = this;
...
ContentLoader.prototype.setUXStateTutorial = function($sidebar) {
  var self = this;
  $sidebar.find("a.cms-link").removeClass("active");
  $sidebar.find("._group-wrap, ._toc").removeClass("is-active");
  var isFound = false;

  $sidebar.find("a.cms-link").each(function(index) {
    var $link = $(this);
...
ContentLoader.prototype.setUXStateArticle = function() {
  $(".articles-item").each(function(index) {
    var $column = $(this);
...
ContentLoader.prototype.setUXStateHome = function() {
  $(".home-section.-posts ._item").each(function(index) {
    var $column = $(this);
...
ContentLoader.prototype.getSectionFragment = function(url) {
  ...
  if (urlParts.length > 1) {
    search = urlParts[1].split("#")[0];
    urlParts = search.split("&");
    _.each(urlParts, function(param) {
      var kv = param.split("=");
...
ContentLoader.prototype.sanitizeUrl = function(href, stripUrlParams) {
...
},{"html5-history-api":1,"lodash":2,"modules/history":7,"modules/search":10,"modules/svg":11,"modules/tenyears":12}],6:[function(require,module,exports){
function Editmode() {
  if (this.isEditMode()) {
    this.bindEditEvents();
    this.bindComponentScrollHelper();
  }
}

Editmode.prototype.isEditMode = function() {
  return $('body').hasClass("edit");
};

Editmode.prototype.bindEditEvents = function() {
  $("body.edit.git-home .tutorial-sidebar").addClass("active-edit");

  $("body.edit.git-home .tutorial-sidebar").on("dblclick", function(e) {
    $(this).toggleClass("active-edit");
  });

  $("body.edit.git-home #header .hamburger, body.edit.git-home #header .primary-nav").on("dblclick", function(e) {
    $("body.edit #header .primary-nav").toggleClass("active-edit");
  });
};

Editmode.prototype.bindComponentScrollHelper = function() {
  var self = this;
  $('body').on('click', function(e) {
    var $elem = $(e.target);

    // if ($elem.closest('.column').length) {
      if (!self.isOnScreen($elem) ) {
        $('html, body').animate({
...
Editmode.prototype.isOnScreen = function(elem){
  var $elem = $(elem);
  var $window = $(window);
...
History.prototype.pushState = function(data, title, url, referrer) {
  if (this.supportsHistory) {
    document.title = title;
    window.history.pushState(data, title, url);

    var getLastPathUrl = function(rawUrl) {
...
History.prototype.pushStateNoTracking = function(data, title, url) {
  if (this.supportsHistory) {
    window.history.pushState(data, title, url);
  }
};
...
Navigation.prototype.bindEvents = function() {
  ...
  $(document).on("click", "#tutorials-nav-mobile h4._nav-item", function(e) {
    $(this).next("._sublist").toggleClass("is-active");
  });

  $(document).on("click", "#tutorials-nav-mobile ._group h5", function(e) {
    $(this).closest("._sublist").find("._group-wrap").removeClass("is-active");
    $(this).next("._group-wrap").addClass("is-active");
  });

};

Navigation.prototype.renderTutorialsNavMobile = function() {
  var self = this;
  var navTemplate = this.getTemplate();
  var navData = this.getSideNavAsData();

  var navElement = _.template(navTemplate);
  $("#tutorials-nav-mobile ._sublist").html(navElement({groups:navData}));

};

Navigation.prototype.getSideNavAsData = function() {
  var result = [];

  $(".tutorial-sidebar ._wrap ._group").each(function() {
    var $group = $(this);
    var groupObj = {
      title: $group.find(".group-link").text(),
      items: []
    };

    var $links = $group.find(".tutorial-link");
    $.each($links, function() {
      var $link = $(this);
      var itemObj = {
        href: $link.attr("href"),
...
Navigation.prototype.getTemplate = function() {
  return '\
  <ol>\
    <% _.each( groups, function( group ){ %>\
    <li class="_group">\
      <h5><%= group.title %></h5>\
      <ol class="_group-wrap">\
...
Newsletter.prototype.bindEvents = function() {
  ...
  $("form.newsletter ._input").on("propertychange change click keyup input paste", function() {
    var email = $(this).val();
    if (!!email.match(/^(.+?)@(.+?)\.(.+?)$/)) {
  ...
  $("form.newsletter ._input").on("focus", function() {
    $(this).parent().addClass("is-focused");
  });

  $("form.newsletter ._input").on("blur", function() {
  ...
  $("form.newsletter").on("submit", function(event) {
    ...
    $form.addClass("form-is-processing");

    var email = $form.find("._input").val();
    var exactTargetUrl = "https://www.atlassian.com/apis/exact-target/${email}/subscribe?mailingListId=" + self.MAILING_LIST_ID;

    exactTargetUrl = exactTargetUrl.replace("${email}", email);
    ...
    $.ajax({
      ...
      success: function(msg){
        ...
        $button.addClass("icon-email-success").removeClass("icon-email-error icon-email-submit");
      ...
      error: function(XMLHttpRequest, textStatus, errorThrown) {
        if (XMLHttpRequest.status === 200) {
          ...
          $form.removeClass("form-is-processing");
          $button.removeClass("icon-email-success").addClass("icon-email-error icon-email-submit");
        }
      }
    });
...
Search.prototype.runSearch = function(term) {
  var self = this;
  var searchResults = [];

  _.each(this.searchData, function(page, pageKey) {
...
Search.prototype.sortByType = function(left, right) {
  return left.flag === "tutorial" ? -1 : 1;
};

Search.prototype.renderSearchResults = function(items) {
  var itemTemplate = this.getItemTemplate();

  var contentTemplate = _.template(itemTemplate);
  $(".search-results-col").html(contentTemplate({items: items}));
};

Search.prototype.getItemTemplate = function() {
  return '\
...
Svg.prototype.processImages = function(data, title, url) {
  ...
  $('img.svg').each(function(){
    ...
    $.get(attr.src, function(data) {

      var $svg = $(data).find('svg');

      // Add replaced image's ID to the new SVG
      if(typeof attr.id !== 'undefined') {
        $svg = $svg.attr('id', attr.id);
      }
      // Add replaced image's classes to the new SVG
      if(typeof attr.className !== 'undefined') {
        $svg = $svg.attr('class', attr.className + ' replaced-svg');
      }
...
Svg.prototype.animateSVG = function() {

  var pathsList = $(".line-to-draw:not(.animate), #Lines path:not(.animate)");

  var animatePath = function(path){
    var length;

    if(path && !path.classList.contains("animate")){
...
function Tenyears() {
  if (!$(".ten-years-index").length || $("body").hasClass("ten-years-bind")) { return; }

  this.bindEvents();
}

Tenyears.prototype.bindEvents = function() {
  var self = this;

  $("body").addClass("ten-years-bind");

  $(document).on("click", ".item .content .info", function(e) {
    e.preventDefault();
    self.renderModal($(this).closest(".content"));
  });

  $(document).on("click", ".ten-years-modal .close", function() {
    var $element = $(this).closest(".modal-layer");
  ...
  // Sources click event
  $(".sources h4").on("click", function() {
    var $parent = $(this).parent();

    $parent.toggleClass("sources-open");
    $parent.find(".source-links").slideToggle();
  });
};

Tenyears.prototype.renderModal = function($element) {
  var modalContent = {
    _date: "",
    _title: "",
    _text: ""
  ...
  modalContent._date = $element.find(".date").text();
  modalContent._title = $element.find("h3:nth-of-type(2)").text();
  modalContent._text = $element.find(".item-modal").text();

  var modalTemplate = this.getModalTemplate();

  var contentTemplate = _.template(modalTemplate);
  $("body").append(contentTemplate(modalContent));
  $(".modal-layer").fadeIn("fast");

};

Tenyears.prototype.getModalTemplate = function() {
  return '\
      <div class="modal-layer portal" style="display:none;">\
        <div class="overlay ten-years-overlay">\
          <div tabindex="-1" class="modal ten-years-modal">\
